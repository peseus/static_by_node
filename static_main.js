...
function createPagebarData (cate_tag, total_page, page_num){
    ...
    pagebar_data.cate_tag = cate_tag;
...
function createPage(cate_tag, total_page, page_num) {
	...
	var dest_dir = proj_path + '/' + cate_tag;
	var dest_path = dest_dir + '/page-' + page_num + '.html';
	try {
...
//	console.log(typeof document.getElementsByClassName('main-body')[0].appendChild);
	...
	for (var i = start_index; i < end_index; i++) {
        ...
        //添加继续阅读
		article_content += "<br/>";
		article_content += "<p><a title=\"\" class=\"btn btn-primary pull-left\" href=\"/detail/" + file + ".html\">继续阅读  →</a> </p><br/> <br/>";
		article_content += "<div class=\"line_dashed\"></div>";
	}
	
	var selector = '.navbar-inner .nav li a[href="/' + cate_tag + '/page-1.html"]';
	...
	del.parentNode.removeChild(del);
	fs.writeFileSync(dest_path, document.innerHTML);
	console.log('写静态页面：' + dest_path + " 完成");
}
		...
		for (var cate_tag in cates) {
			if (cates.hasOwnProperty(cate_tag)) {
				...
				for (var page_num = 1; page_num <= total_page; page_num++) {
					createPage(cate_tag, total_page, page_num);
				}
			}
		}
	}
